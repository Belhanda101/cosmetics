...
def profile_update(request):
    profile = Profile.objects.get(user__username = request.user.username)
    # profile update
    update = ProfileForm(instance=request.user.profile)#instantiate the profile update form for a GET request
    if request.method =='POST':
        update = ProfileForm(request.POST, request.FILES, instance=request.user.profile)#instantiate the profile update form for a GET request
        if update.is_valid():
            update.save()
...
def pay(request):
    if request.method == 'POST':
        api_key = 'sk_test_4b705ad637039479cc5573b06b873c01228606e0'
        curl = 'https://api.paystack.co/transaction/initialize'
        cburl = 'http://54.82.37.51/callback'
        ref =  str(uuid.uuid4())
